...
public class JLife : MonoBehaviour
{
    ...
    public void movementFunction()// The Walk Checker
    {
        if (mode == 1 && _nx != -1 && _ny != -1 && (!(_nx == _x && _ny == _y)))
        {
            ...
            {
                suddenModelHalt(1);   // For halting the unit after reaching the point                     
            }
            if (isContinuity)
            {
    ...
    public void showHealthBar(int[] _data)// Helper Octoman
    {
        if (_data[1] > 0)// I used brilliant skip , when max health == 0 means only to look at camera
        {
            ...
            if (health < 0.40f)
            {
                healthDisplay.GetComponent<SpriteRenderer>().color = Color.red;
            }
            else if (health < 0.70f)
            {
                healthDisplay.GetComponent<SpriteRenderer>().color = Color.yellow;
            }
            else
            {
                healthDisplay.GetComponent<SpriteRenderer>().color = Color.green;
            }
            healthDisplay.localScale = new Vector3(health * 0.181f, 1f * 0.091f, 1f * 0.147f );// health = 0f....1f                            
            // the above hand coded multipliers are the default scale of healthbar
        }
        else if (_data[1] == 0)
        {
            healthBarLookAtCamera();
    ...
    {
        Color k23 = healthDisplay.GetComponent<SpriteRenderer>().color;
        k23.a = 255;//actually 255
        healthDisplay.GetComponent<SpriteRenderer>().color = k23;
    }

    public void healthBarLookAtCamera()
    {
        healthDisplay.LookAt(healthDisplay.position + (myCamera.transform.rotation * Vector3.back),
            myCamera.transform.rotation * Vector3.up
            );
    }

    public void suddenModelHalt( int _postMode )
    {
        sudIdleCounter++;
        if (sudIdleCounter % 3 == 1)
        {            
            anim.SetInteger(controlParam, (0 * 10) + _postMode );//01              
            whenNotInUpdate();
